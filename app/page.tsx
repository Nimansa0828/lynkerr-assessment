...
export default function Home() {
  ...
  const handleLike = async (e: React.MouseEvent, itemId: string, currentLikes: number) => {
    e.preventDefault(); 
    ...
    if (error) {
      ...
      setListings((prev) =>
        prev.map((item) => (item.id === itemId ? { ...item, likes: newLikes } : item))
      );
    }
  };
            ...
            <Link href={`/listings/${item.id}`} key={item.id} className="group">
              <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
                <div className="relative">
                  ...
                  {}
                  <button 
                    onClick={(e) => handleLike(e, item.id, item.likes)}
                    className="absolute top-4 right-4 bg-white/90 backdrop-blur p-2 rounded-full shadow-md hover:scale-110 transition-transform active:scale-95 group/heart"
                  >
                    <span className="flex items-center space-x-1">
                      <span className="text-red-500">❤️</span>
                      <span className="text-xs font-bold text-gray-700">{item.likes || 0}</span>
                    </span>
                  </button>
